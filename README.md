...
|---|---|
| `config-server` | Serves centralized, environment-specific configuration to all other services. Config files are sourced from [eCommerce-config-server](https://github.com/KamaliyaVishal/eCommerce-config-server). |
...
Shared and service-specific configuration is managed through the `config-server` module. Each service reads its properties from there on startup rather than bundling all settings locally.

The actual `.yaml` configuration files (per service, per environment) are kept in a separate Git-backed config repository rather than in this repo:

> 📄 **[eCommerce-config-server](https://github.com/KamaliyaVishal/eCommerce-config-server)** — holds `application.yaml`, `api-gateway.yaml`, `inventory-service.yaml`, `order-service.yaml`, `order-service-dev.yaml`, etc., and documents how the `config-server` module here is wired up to pull from it.

In short:
- This repo's `config-server` module is the running Spring Boot service.
- The [eCommerce-config-server](https://github.com/KamaliyaVishal/eCommerce-config-server) repo is the Git source it pulls configuration from (via `spring.cloud.config.server.git.uri`).
- Each downstream service (`api-gateway`, `inventory-service`, `order-service`) then fetches its own settings from the running Config Server on startup.
